...
metadata:
  ...
  enhancements:
    ...
    - Optional Serena-assisted symbol navigation and index checks
---
...
4. If Serena MCP tools are available, call `initial_instructions` before source-code discovery. For large or newly checked out projects, or when project instructions mention Serena index maintenance, load [`references/serena-navigation.md`](references/serena-navigation.md) and follow its index and navigation guidance.

### Optional Serena-Assisted Navigation

Use Serena as a code-navigation accelerator, not as a replacement for evidence.
It is especially useful for Java, Kotlin, Python, TypeScript, C#, and other
symbol-rich projects.

- Prefer `get_symbols_overview` on the smallest relevant source file before reading whole implementation files.
- Use `find_symbol` when a class, method, function, or record name is known; include symbol bodies only when needed.
- Use `search_for_pattern` when the symbol name or location is uncertain; keep normal shell/`rg` search for docs, YAML, static assets, generated manifests, and broad file discovery.
- Use `find_referencing_symbols` before documenting shared APIs, runtime graph nodes, state keys, adapters, or boundary objects whose usage matters to architecture.
- Treat Serena output as navigation evidence only. Every documented claim still needs traceable support from source files, config, tests, or terminal output.
- If Serena is unavailable, stale, or noisy, continue with `scan.py`, `rg`, targeted file reads, and executable tests.
...
| [`references/serena-navigation.md`](references/serena-navigation.md) | Phase 1/2 — only when Serena MCP tools are available and source-code symbol navigation will help |
